...
if env_args["n_edges"] == -1:
    env_args["n_edges"] = int(
        (env_args["n_nodes"] * (env_args["n_nodes"] - 1) // 2) * 0.30
    )
...
if __name__ == "__main__":
    ...
    run_name = (
        f"run_{int(time.time())%1e7}_{env_id}_{model_type}_N{env_args['n_nodes']}_E{env_args['n_edges']}_Parenting{env_args['parenting'] if 'parenting' in env_args else 'NA'}"
    ...
    for i in range(env_args["n_nodes"], env_args["n_nodes"] + 1, 5):
        ...
        print(
            ...
            model_type,
            " || Parenting: ",
            env_args["parenting"] if "parenting" in env_args else "NA",
        )
        print("*-" * 10)
