...
class MulticastONREnv(gym.Env):
    ...
    def __init__(self, config, is_eval_env=False) -> None:
        super(MulticastONREnv, self).__init__()

        self.config = config
        parenting = config.parenting

        if parenting not in [0, 1, 2, 3, 4]:
        ...
        self.observation_space = gym.spaces.Box(
            ...
            high=1000,
            shape=(
                (4 + fe.get_num_features()) * self.n_nodes
                + self.n_edge_features * self.n_edges * 2
                + 2 * self.n_edges * 2,
            ),
        )
    ...
    def reset(self, seed=None, options={}) -> np.array:
        ...
        self.src = 0
        self.dests = np.random.choice(
            np.arange(1, self.n_nodes), size=self.n_dests, replace=False
        )
        ...
        max_distance = (
            np.random.rand() * (farthest_node - farthest_target) + farthest_target
        )
        ...
        edge_f = np.array(
            [G[u][v]["delay"] for u, v in G.edges], dtype=np.float32
        ).reshape(-1, 1)

        edge_f = np.concatenate(
            (edge_f, np.zeros((2 * self.n_edges, 1), dtype=np.float32)), axis=1
        )

        self.graph = gym.spaces.GraphInstance(
            nodes=x, edges=edge_f, edge_links=edge_index
        )
    ...
    def _get_mask(self) -> np.array:
        ...
        if self.parenting >= 2:
            mask[
                self.graph.nodes[self.graph.edge_links[:, 0], self.NODE_HAS_MSG] < 0.5
            ] = False
            mask[
                self.graph.nodes[self.graph.edge_links[:, 1], self.NODE_HAS_MSG] > 0.5
            ] = False
        ...
        if self.parenting >= 3:
            ...
            for id, v in enumerate(Vs):
                ...
                edges_to_tree = neighbor_edges & (
                    self.graph.nodes[self.graph.edge_links[:, 0], self.NODE_HAS_MSG]
                    > 0.5
                )
                distances = (
                    self.graph.nodes[
                        self.graph.edge_links[edges_to_tree, 0],
                        self.NODE_DISTANCE_FROM_SOURCE,
                    ]
                ...
                all_distances = (
                    self.graph.nodes[
                        self.graph.edge_links[:, 0], self.NODE_DISTANCE_FROM_SOURCE
                    ]
    ...
    def step(
    ...
    ) -> Tuple[gym.spaces.GraphInstance, SupportsFloat, bool, bool, dict]:
        ...
        assert (
            np.isclose(self.graph.edges[action, self.EDGE_IS_TAKEN], 1) == False
        ), f"BUG IN THE LIBRARY! Edge {action} is already a part of the path!"

        if self.parenting >= 2:
            assert np.isclose(
                self.graph.nodes[u, self.NODE_HAS_MSG], 1
            ), f"BUG IN THE LIBRARY! Node {u} is not a part of the path!"
            assert np.isclose(
                self.graph.nodes[v, self.NODE_HAS_MSG], 0
            ), f"BUG IN THE LIBRARY! Node {v} is already a part of the path!"
        ...
        if np.isclose(self.graph.nodes[u, self.NODE_HAS_MSG], 0) or np.isclose(
        ...
        if self.graph.nodes[v, self.NODE_IS_TARGET] == 1:
            if (
                self.graph.nodes[v, self.NODE_DISTANCE_FROM_SOURCE]
                > self.graph.nodes[v, self.NODE_MAX_DISTANCE] + 1e-4
            ):
                reward = -2 * self.n_nodes * self.n_dests
                done = True
                info["solved"] = False
        ...
        dests_left = np.logical_and(
            self.graph.nodes[:, self.NODE_HAS_MSG] < 1e-5,
            self.graph.nodes[:, self.NODE_IS_TARGET] > (1 - 1e-5),
        ).sum()
        info["mask"] = self._get_mask()
        ...
        if dests_left == 0:
            ...
            assert (
                self.constraints_satisfied == self.n_dests
            ), "BUG IN THE LIBRARY! Not all constraints satisfied!"
            info["solved"] = True
...
register(
    id="MulticastONREnv-v0",
    entry_point="multicast_env_onr:MulticastONREnv",
)
