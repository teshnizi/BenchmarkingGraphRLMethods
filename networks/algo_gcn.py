...
class AlgoGCN(torch.nn.Module):
    def __init__(self, node_f: int, edge_f: int, action_type: str, config) -> None:
        ...
        self.gnn_layers = torch.nn.ModuleList([
            pyg.nn.GCNConv(in_channels=self.config.hidden, 
                             out_channels=self.config.hidden,
            )
            for _ in range(config.layers)])

        self.edge_w_layers = torch.nn.ModuleList([torch.nn.Linear(self.config.hidden, 1) for _ in range(config.layers)])
    ...
    def encode(self, batch_graph: pyg.data.Batch):
        ...
        for i in range(self.config.layers):
            ...
            edge_weights = self.edge_w_layers[i](edge_features).squeeze(-1)
            edge_weights = torch.sigmoid(edge_weights)
            
            x = x + self.gnn_layers[i](x, edge_index, edge_weights)
