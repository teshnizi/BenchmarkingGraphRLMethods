...
class GNN(torch.nn.Module):
    def __init__(self, node_f: int, edge_f: int, action_type: str, config) -> None:
        ...
        self.gnn_layers = torch.nn.ModuleList([
            # pyg.nn.GCNConv(in_channels=self.config.hidden, out_channels=self.config.hidden, cached=True) for _ in range(config.layers)])
        ...
        if self.action_type == 'node':
            self.action_net = ResidualMLP(self.config.hidden, self.config.hidden, 1, config.activation, 2, norm=False, dropout=config.dropout)
