...
def get_model(model_type, model_config, env_id):
    ...
    if model_type == "GNN":
        return gnn.GNN(
            node_f=node_f, edge_f=edge_f, action_type=action_type, config=model_config
        )
        ...
        return algo_gcn.AlgoGCN(
            node_f=node_f, edge_f=edge_f, action_type=action_type, config=model_config
        )
    elif model_type == "GNN_GAT":
        return algo_gat.AlgoGAT(
            node_f=node_f, edge_f=edge_f, action_type=action_type, config=model_config
        )
    elif model_type == "GNN_GTN":
        return algo_gtn.AlgoGraphTransformer(
            node_f=node_f, edge_f=edge_f, action_type=action_type, config=model_config
        )
    elif model_type == "Graphormer":
        return graphormer.GraphormerAgent(
            node_f=node_f, edge_f=edge_f, action_type=action_type, config=model_config
        )
    else:
...
def forward_pass(
    ...
    if model_type.startswith("GNN"):
        ...
        for i in range(x.shape[0]):
            ...
            dp["attn_edge_type"] = (
                torch.Tensor(dp["attn_edge_type"]).unsqueeze(0).long()
            )
    ...
    if has_mask:
        action, logprob, entropy, value = model(
            model_input, masks, actions, pick_max=pick_max
        )
    else:
...
def draw_graph(G, file_name, edges_taken=[]):
    ...
    if len(edges_taken) == 0:
        edge_color = [
            "orange"
            if (
                G.edges[(u, v)]["edge_attr"][1] == 1
                or G.edges[(v, u)]["edge_attr"][1] == 1
            )
            else "black"
            for (u, v) in G.edges
        ]
        ...
        edge_color = [
            "orange" if ((u, v) in edges_taken) or ((v, u) in edges_taken) else "black"
            for (u, v) in G.edges
        ]
    ...
    nx.draw(
        G, pos=layout, with_labels=True, edge_color=edge_color, node_color=node_color
    )
