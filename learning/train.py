...
def train_ppo(
    ...
    obs_stack = torch.zeros(
        (train_config.n_steps, train_config.n_envs)
        + envs.single_observation_space.shape
    ).to(device)
    if train_config.has_mask:
        masks_stack = (
            torch.zeros(
                (train_config.n_steps, train_config.n_envs) + train_config.mask_shape
            ).to(device)
            < 1
        )
    else:
    ...
    action_stack = torch.zeros(
        (train_config.n_steps, train_config.n_envs) + envs.single_action_space.shape
    ).to(device)
    ...
    for update in range(train_config.num_updates):
        if update % train_config.eval_freq == 0:
            ...
            learning.eval.eval_model(
                ...
                has_mask=train_config.has_mask,
                device=device,
        ...
        for step in range(train_config.n_steps):
            ...
            with torch.no_grad():
                x, edge_features, edge_index = graph_envs.utils.devectorize_graph(
                    next_obs, env_id, **env_args
                )
                action, logprob, entropy, value = utils.forward_pass(
                    model,
                    model_type,
                    x,
                    edge_features,
                    edge_index,
                    train_config.has_mask,
                    next_mask,
                    actions=None,
                )
        ...
        print("----------------------------------")
        print(
            f"Update: {update}, Mean Ep Rew: {np.mean(ep_rews)}, Mean Ep Len: {np.mean(ep_lens)}, Sample_size: {len(ep_rews)}"
        )
        print(
            f"Ep Rew std: {np.std(ep_rews)}, Ep Len std: {np.std(ep_lens)}", flush=True
        )
        ...
        with torch.no_grad():
            ...
            _, _, _, next_value = utils.forward_pass(
                model,
                model_type,
                x,
                edge_features,
            ...
            if train_config.gae:
                ...
                for t in reversed(range(train_config.n_steps)):
                    ...
                    delta = (
                        rewards_stack[t]
                        + train_config.gamma * nextvalues * nextnonterminal
                        - values_stack[t]
                    )
                    advantages_stack[t] = lastgaelam = (
                        delta
                        + train_config.gamma
        ...
        for epoch in range(train_config.n_epochs):
            ...
            for start in range(
                0, train_config.batch_size, train_config.mini_batch_size
            ):
                end = start + train_config.mini_batch_size
                mb_inds = b_inds[start:end]

                x, edge_features, edge_index = graph_envs.utils.devectorize_graph(
                    b_obs[mb_inds], env_id, **env_args
                )
                _, new_logprobs, entropy, new_values = utils.forward_pass(
                    model,
                    model_type,
                    x,
                    edge_features,
                ...
                with torch.no_grad():
                    ...
                    clipfracs += [
                        ((ratio - 1).abs() > train_config.clip_coef)
                        .float()
                        .mean()
                        .item()
                    ]
                ...
                pg_loss2 = -mb_advantages * ratio.clamp(
                    1 - train_config.clip_coef, 1 + train_config.clip_coef
                )
                ...
                if train_config.clip_value_loss:
                    ...
                    ).clamp(-train_config.clip_coef, train_config.clip_coef)
                ...
                loss = (
                    pg_loss.mean()
                    + train_config.value_loss_coef * value_loss
                    - train_config.entropy_loss_coef * entropy_loss
                )
        ...
        writer.add_scalar(
            "charts/sps",
            train_config.n_steps * train_config.n_envs / (time.time() - t_start),
            global_step,
        )
